...
#include <cstring>  // For string manipulation functions
 ...
 // Constants
 constexpr const char* SERVER_ADDRESS = "192.168.1.5";
 ...
 -----END EC PRIVATE KEY-----)";
         ...
         while (true) {
             ...
             if (command == "HELO") {
                 ...
                 // Respond with user's full name (replace with your actual name)
                 std::string response = sha1(authdata + args[1]) + " " + "John Doe\n";
                 ssl_write(ssl, response);
             } 
         ...
         if (ssl)
         {
            //SSL_shutdown(ssl);
            //SSL_free(ssl);
            std::cerr << "Exception:SOMETHING FISHY SSL CLEAN UP ACTIVITY "<< std::endl;
         }
